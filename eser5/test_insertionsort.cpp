#include "sorts.hpp"
#include <iostream>
#include <concepts>
#include <random>
#include "randfiller.h"

using namespace std;

int main() {
    randfiller rf; 
    vector<int> dim(100); 
    rf.fill(dim,100,8000);

    for (int i=0; i<100;i++){
        
        vector<int> casuali(dim[i]); //così mi creo un vettore fatto da tanti elementi quanti il numero in posizione i-esima di dim

        rf.fill(casuali,1,500);//riempilo con numeri tra 1 e 500

        //metti in ordine
        insertion_sort(casuali);

        //vedi se funziona
        //is_sorted(casuali);
        if (!is_sorted(casuali)){
            cerr<<"l'insertion non ha fatto il suo lavoro \n";
            return EXIT_FAILURE;
        }
    }
    cout<<"l'insertion ha fatto il suo lavoro sui numeri\n";

    vector<string> stringa={"ciao", "mamma", "papa", "nonna", "nonno", "cugina", "zio", "parente", "nicole", "conni"};
    insertion_sort(stringa);

    if (!is_sorted(stringa)){
            cerr<<"l'insertionsort non ha fatto il suo lavoro sulle stringhe \n";
            return EXIT_FAILURE;
        }
    cout<<"l'insertionsort ha fatto il suo lavoro sulle stringhe\n";
    
    return EXIT_SUCCESS;
}

